...
**类型**：PRD / 功能规格书
**版本**：v3.0
**参与成员**：析客（需求分析师）、瑞思（用户研究员）、竞析（竞品分析师）、数析（数据分析师）、路径（路线图规划师）、方向明（主理人·审查汇编）
**本版由产品战略团队产出**：基于需求分析、用户研究、竞品分析、数据分析与路线图规划，经主理人审查汇编成稿，作为对外、对开发团队、对 AI Agent 的权威功能规格参考。

---

## 📌 TL;DR（执行摘要）

- **核心目标**：将微博、知乎、B站、抖音、百度、今日头条六大平台热搜聚合到一屏，通过跨平台综合热榜发现"多平台共同热门"话题，支持搜索、组合筛选与个性化推荐。
- **关键决策**：
  - 登录方式采用**独立账号体系**（邮箱注册 + 密码登录 + 邮箱验证 + 邮件链接密码找回），引入 SQLite 存储用户数据，游客模式 100% 可用。
  - 数据源主路径为六大平台直连公开数据端点（零成本、可控），uapis.cn 等第三方聚合仅作兜底，Mock 数据仅用于开发/联调或全源失败兜底。
  - 综合榜排序算法：同一话题的综合热度 = 各平台归一化热度（0-100）的最大值 × 共现加成（共现平台≥3 时 ×1.2，≥2 时 ×1.1）。
  - 安全合规与异常/缓存基座作为全局基础保障模块，所有功能模块共用，不再各自实现。
- **下一步**：按 M0-M5 里程碑推进，优先完成项目底座与核心聚合功能（M0/M1），独立账号体系与合规基座在 M0/M3 阶段实现（M0 先行落地合规与异常基座，较原方案增加约 5 个工作日账号工作量 + 基础保障模块工作量）。
...
| 项目 | 内容 |
|------|------|
| 推荐方案 | 独立邮箱账号体系（SQLite + PBKDF2 + 阿里云邮件推送 / 腾讯云 SES + Turnstile + JWT/Refresh Token） |
| 优先级 | 必须做（MVP 必须） |
| 预期影响 | 解决 GitHub OAuth 对普通网民的"错配"问题，扩大目标用户覆盖面 |
| 资源需求 | SQLite（本地文件，WAL）+ Redis + 阿里云邮件推送/腾讯云 SES（免费额度）+ Turnstile（免费）；限流计数器采用 进程内 LRU+Redis 双层（单机可靠，Redis 跨重启/多实例共享；强一致由 SQLite 承担） |
| 风险等级 | 中（打破原"无数据库"优势，新增约 5 个工作日账号 + 基础保障模块工作量） |
...
本版由产品战略团队产出，涵盖需求分析、用户研究、竞品分析、数据分析、路线图规划与主理人审查汇编：

- **析客（需求分析师）**：基于研究文档与三方产出，撰写完整 PRD（核心功能 + 9 个 MVP 模块 + 10 个后续方向），覆盖接口设计、异常处理、缓存策略、安全要求。
- **瑞思（用户研究员）**：综合用户研究数据，确认独立账号体系对核心用户群（普通网民）的战略正确性，提出游客零门槛、渐进式注册、最小化字段、多设备同步等关键建议。
- **竞析（竞品分析师）**：调研同类产品账号体系，输出独立账号技术方案（SQLite 存储 + PBKDF2-SHA256 哈希 + 阿里云邮件推送 / 腾讯云 SES 邮件 + Turnstile 人机验证 + JWT/Refresh Token + Redis 限流），评估工作量增量 +5 工作日。
- **数析（数据分析师）**：整理产品指标基线，含独立账号新增指标（注册转化率/登录成功率/密码找回率/验证码送达率/账号安全指标）与各核心模块缓存覆盖建议、接口性能基线。
- **路径（路线图规划师）**：基于 PRD 评估优先级与时间线（M0-M5 里程碑，见行动清单）。
- **研究文档**：RESEARCH（2026-07-30，27 章覆盖背景/调研/目标/范围/品牌/架构/数据模型/适配器契约/API/缓存/搜索/登录/异常/非功能/安全/可观测性/测试/验收/风险/里程碑）。
